...
public class Tube {
    ...
    public Tube(int tubeCount, int tubeIdx) {
        ...
        textureUpperTube = new Texture("tube\\tube_flipped.png");
        textureDownTube = new Texture("tube\\tube.png");
    ...
    public boolean isHit(Bird bird){
        if (bird.getY() <= gapY - gapHeight / 2 && bird.getX() + bird.width >= x && bird.getX() <= x + width)
            return true;
        if (bird.getY() + bird.height >= gapY + gapHeight / 2 && bird.getX() + bird.width >= x && bird.getX() <= x + width)
            return true;
        return false;
    }
